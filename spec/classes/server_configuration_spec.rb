  ...
  module Bolt
    describe Bolt do
      ...
      describe '.configure_server' do
        before { Sidekiq.options[:additional_scheduled_enqs] = %w(SomeEnq) }
        after { Sidekiq.options[:additional_scheduled_enqs] = nil }

        it 'should initialize a Feed with the sidekiq options' do
          expect(Feed).to receive(:new).with(Sidekiq.options)
